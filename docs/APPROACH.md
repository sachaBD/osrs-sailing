...
on. This says how to attack it, and in what order. What actually happened when
we ran it is in `RESULTS.md` - kept apart deliberately, so the plan stays
readable as a plan.
...
Parameters come from `../model/params.tsv` so the guessed constants stay swappable.
...
Built, and measured in `RESULTS.md`: search reaches about 6% over greedy, an
independent rollout baseline agrees with it, and giving the planner *more*
true information still makes it worse - which is now the argument for Layer 5

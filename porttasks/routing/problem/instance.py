...
from ..world.catalogue import Catalogue
...
class Instance:
    ...
    @classmethod
    def at_level(cls, level: int, world: Catalogue | None = None,
                 matrix: Distances | None = None, params: Params | None = None) -> Instance:
        ...
        size = len(names)
        if not size:
            raise ProblemError(f'no port is dockable at level {level}')

...
also why the lattice cannot thread a narrow strait: tables/waypoints.tsv is where
...
from porttasks.paths import SEA_GRAPH as CACHE
...
class SeaGraph:
    ...
    def load(cls, path: str = CACHE) -> SeaGraph:
        try:
            ...
            raise SurveyError(
                f'{path} not found; build it with `python3 -m porttasks.routing.world.survey.build`') from exc
        return cls(
...
def _berth(mask: WaterMask, x: int, y: int, name: str, limit: int = 60) -> tuple[int, int]:
    ...
    raise SurveyError(f'{name} is more than {limit} tiles from open sea')
...
def build(mask: WaterMask, world: Catalogue, spacing: int = SPACING,
    ...
    for x, y, note in handpoints.load():
        ...
        if not mask.sea_at(row, col):
            ...
            raise SurveyError(f'{handpoints.PATH}: waypoint {where} is not on open water')
        hand.append((row, col))
    nodes.extend(hand)
    ...
    for name, berth in berths.items():
        ...
        if not found:
            raise SurveyError(f'{name} cannot reach any waypoint by sea')
    ...
    for i in hand_ids:
        if not edges[i]:
            ...
            raise SurveyError(f'{handpoints.PATH}: waypoint {x},{y} connects to nothing - '
    ...
    if len(set(port_labels.values())) > 1:
        ...
        raise SurveyError('ports fall in separate components: '

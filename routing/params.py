"""Read params.tsv and transport.tsv, the hand-editable parameter tables."""
...
PARAMS = 'routing/params.tsv'
TRANSPORT = 'routing/transport.tsv'


def _rows(path: str, fields: list[str]) -> list[dict[str, str]]:
    with open(path) as f:
        lines = [ln.rstrip('\n') for ln in f if ln.strip() and not ln.startswith('#')]
    if not lines:
        raise ModelError(f'{path} has no rows')
    header = lines[0].split('\t')
    if header[:len(fields)] != fields:
        raise ModelError(f'unexpected header in {path}\n  found:    {header}\n'
                         f'  expected: {fields}')
    out = []
    for line in lines[1:]:
    ...
    def capacity_at(self, level: int) -> int:
        ...
        for threshold, tasks in self.capacity:
            if level >= threshold:
                held = tasks
        if not held:
            raise ModelError(f'no task capacity defined at level {level}')
        return held

    @classmethod
    def load(cls, path: str = PARAMS) -> Params:
        ...
        if not ladder:
            raise ModelError(f'{path} defines no capacity_at_* rows')

        def ticks(name: str) -> int:
            value = raw.get(name, '?')
            if value == '?':
                raise ModelError(f'{path}: {name} is still unmeasured; give it a placeholder')
            return int(value)
...
def charter_ports(path: str = TRANSPORT) -> set[str]:

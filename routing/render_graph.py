"""Draw the sea graph over the world map, so its shortcuts can be eyeballed.
    ...
    python3 -m routing.render_graph                     the graph alone
    python3 -m routing.render_graph --tiles             over the real map tiles
    python3 -m routing.render_graph Catherby Aldarin    plus that route
    python3 -m routing.render_graph --lanes             every pair's exact route
...
OUT = 'routing/renders/graph.png'
ROUTES_FILE = 'routing/cache/port_routes.json'
...
def shortest_path(graph: SeaGraph, origin: str, destination: str) -> tuple[list[int], float]:
    ...
    for name in (origin, destination):
        if name not in graph.berths:
            raise ChartError(f'unknown port {name!r}')
    ...
    if goal not in dist:
        raise ChartError(f'no charted route {origin} -> {destination}')

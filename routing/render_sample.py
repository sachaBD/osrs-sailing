"""A contact sheet of sampled routes, one per panel, for checking by eye.
    ...
    python3 -m routing.render_sample            25 routes, seed 0
    python3 -m routing.render_sample 12 7       12 routes, seed 7
...
from . import portmatrix
from .portmatrix import PortMatrix
from .render_graph import _FONT, _basemap
from .world import World

OUT = 'routing/renders/routes_sample.png'
...
def main(argv: list[str]) -> None:
    ...
    matrix = PortMatrix.load()
    world = World.load()
    mask = watermask.build()
    with open(portmatrix.ROUTES) as f:
        routes = json.load(f)

...
def level_at(xp: int) -> int:
    ...
    if xp < 0:
        raise WorldError(f'negative xp: {xp}')
    level = 1
...
def xp_for_level(level: int) -> int:
    if not 1 <= level <= 99:
        raise WorldError(f'level out of range: {level}')
...
class World:
    ...
    @classmethod
    def load(cls, tasks_path: str = parse_list.JSON_OUT,
             locations_path: str = locations.PATH) -> World:
        ports = {}
        for name, record in locations.load(locations_path).items():
            if not record.get('coords'):
                raise WorldError(f'{name} has no coords; the sea chart needs them')
        ...
        for row in rows:
            for key in ('noticeBoard', 'from', 'to'):
                if row[key] not in ports:
                    raise WorldError(f'task {row["id"]} names unknown port {row[key]!r}')
            tasks[row['id']] = Task(

...
def _mosaic(tile_dir: str, zoom: int) -> tuple[np.ndarray, tuple[int, int]]:
    bounds = mapdata.bounds()['zooms'][zoom]
    ...
    if not paths:
        raise ChartError(f'no tiles in {tile_dir}/{zoom}; run `make tiles`')
    for path in paths:
...
def build(tile_dir: str = mapdata.TILE_DIR, zoom: int = 0) -> WaterMask:
